...
from ..exceptions import (
    DbxConfigurationError,
    DbxAuthenticationError,
    DbxExecutionError,
)
...
class DatabricksAdapter(BaseAdapter):
    ...
    def _ensure_connection_params(self):
        ...
        for req in required:
            if req not in self.config:
                raise DbxConfigurationError(
                    f"Databricks config missing required key: {req}"
                )
    ...
    def get_metadata(self, catalog: str, schema: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in rows:
                meta[
                    row.model_name
                ] = {  # Row objects in databricks-sql usually accessible by attribute or index?
    ...
    def get_next_execution_id(self, catalog: str, schema: str) -> int:
        try:
            rows = self.fetch_result(
                f"SELECT MAX(execution_id) FROM {catalog}.{schema}._dbx_model_metadata"
            )
    ...
    def _ensure_metadata_table(self, catalog: str, schema: str):
        ...
        try:
            self.execute(
                f"ALTER TABLE {catalog}.{schema}._dbx_model_metadata ADD COLUMNS (execution_id BIGINT)"
            )
        except Exception:
            pass
