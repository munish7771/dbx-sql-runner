...
class DbxRunner:
    ...
    def run(self, preview=False):
        try:
            ...
            for model in sorted_models:
                ...
                target_context = {
                    m: f"{self.catalog}.{self.schema}.{model_map[m].name}"
                    for m in model_map
                }
                ...
                current_hash = hashlib.sha256(
                    current_sql_content.encode("utf-8")
                ).hexdigest()
                ...
                if action == "EXECUTE":
                    if model.materialized == "ddl":
                        ...
                        context_map[model.name] = (
                            f"{self.catalog}.{self.schema}.{model.name}"
                        )
                    else:
                        # Staging FQN: suffix with __staging
                        context_map[model.name] = (
                            f"{self.catalog}.{self.schema}.{model.name}__staging"
                        )
                else:
                    context_map[model.name] = (
                        f"{self.catalog}.{self.schema}.{model.name}"
            ...
            for item in execution_plan:
                ...
                for dep in model.depends_on:
                    if dep in model_status and model_status[dep] in [
                        "ERROR",
                        "SKIP_UPSTREAM",
                    ]:
            ...
            for item in execution_plan:
                ...
                try:
                    ...
                    results["ERROR"] += (
                        1  # Should we count promotion error as error? Yes.
                    )
            ...
            logger.info(
                f"Done. PASS={results['PASS']} WARN={results['WARN']} ERROR={results['ERROR']} SKIP={results['SKIP']} TOTAL={results['PASS'] + results['ERROR'] + results['SKIP']}"
            )
    ...
    def _log_start(self, idx, total, model):
        ...
        logger.info(
            f"{idx} of {total} START sql {model.materialized} model {self.catalog}.{self.schema}.{model.name} ... [RUN]"
        )

    def _log_end(self, idx, total, model, duration):
        logger.info(
            f"{idx} of {total} OK created {model.materialized} model {self.catalog}.{self.schema}.{model.name} ... [OK in {duration:.2f}s]"
        )
    ...
    def _promote_model(self, model: Model):
        ...
        if model.materialized == "view":
            ...
            target_context = {
                m.name: f"{self.catalog}.{self.schema}.{m.name}"
                for m in self.loader.load_models()
            }
            ...
            try:
                self.adapter.execute(
                    f"ALTER TABLE {fqn_staging} RENAME TO {fqn_target}"
                )
            except Exception as e:
                logger.warning(
                    f"Warning: Could not rename DDL artifact {fqn_staging}. Error: {e}"
                )
