...
class MockAdapterLogic(MagicMock):
    ...
    def ensure_schema_exists(self, c, s):
        pass

    def drop_schema_cascade(self, c, s):
        pass

    def update_metadata(self, c, s, name, hash, mat, eid):
        pass
...
class TestRunnerLogic(unittest.TestCase):
    ...
    def test_skip_view_logic(self):
        ...
        self.adapter.metadata = {
            "my_view": {"sql_hash": expected_hash, "materialized": "view"}
        }
        ...
        create_calls = [
            s for s in self.adapter.executed_sql if "CREATE OR REPLACE" in s
        ]
    ...
    def test_execute_table_even_if_hash_matches(self):
        ...
        expected_hash = hashlib.sha256(sql_content.encode("utf-8")).hexdigest()
        self.adapter.metadata = {
            "my_table": {"sql_hash": expected_hash, "materialized": "table"}
        }
        ...
        # Verify EXECUTION
        create_calls = [
            s for s in self.adapter.executed_sql if "CREATE OR REPLACE TABLE" in s
        ]
    ...
    def test_failure_cleanup(self):
        ...
        try:
            ...
            self.fail(
                f"runner.run() should handle exceptions internally but raised: {e}"
            )
        ...
        self.assertTrue(
            drop_calls, f"Cleanup didn't attempt to drop staging table. Calls: {calls}"
        )
    ...
    def test_execute_ddl_with_this_variable(self):
        ...
        self.assertIn(
            expected_sql,
            executed_sqls,
            f"Did not find expected DDL execution. Found: {executed_sqls}",
        )
    ...
    def test_source_injection(self):
        ...
        create_view_calls = [
            s for s in self.adapter.executed_sql if "CREATE OR REPLACE VIEW" in s
        ]
