...
class MockAdapter(BaseAdapter):
    ...
    def update_metadata(
        self, catalog, schema, model_name, sql_hash, materialized, execution_id
    ):
        pass
...
class TestModularRunner(unittest.TestCase):
    ...
    def test_run_order_and_execution(self):
        ...
        upstream_build = [
            s for s in sqls if "CREATE OR REPLACE TABLE cat.sch.upstream__staging" in s
        ]
        ...
        downstream_build = [
            s for s in sqls if "CREATE OR REPLACE VIEW cat.sch.downstream__staging" in s
        ]
        ...
        promote_upstream = [
            s
            for s in sqls
            if "ALTER TABLE cat.sch.upstream__staging RENAME TO cat.sch.upstream" in s
        ]
        ...
        promote_downstream = [
            s for s in sqls if "CREATE OR REPLACE VIEW cat.sch.downstream" in s
        ]
        ...
        cleanup_up = [
            s for s in sqls if "DROP TABLE IF EXISTS cat.sch.upstream__staging" in s
        ]
