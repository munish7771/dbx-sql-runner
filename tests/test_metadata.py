...
class TestMetadataUpdate(unittest.TestCase):
    ...
    def test_ensure_metadata_table(self):
        ...
        self.assertIn(
            "execution_id BIGINT",
            create_call[0],
            "execution_id BIGINT missing in CREATE TABLE",
        )
        ...
        self.assertIn(
            "ADD COLUMNS (execution_id BIGINT)",
            alter_call[0],
            "Incorrect ALTER TABLE statement",
        )

    def test_update_metadata(self):
        execution_id = 123
        self.adapter.update_metadata(
            "cat", "sch", "my_model", "hash123", "view", execution_id
        )
        ...
        sql = insert_call[0]
        self.assertIn(
            f", {execution_id})", sql, "execution_id value missing or quoted in INSERT"
        )
